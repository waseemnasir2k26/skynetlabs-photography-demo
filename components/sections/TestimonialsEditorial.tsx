...
export function TestimonialsEditorial() {
  ...
  return (
    <section className="py-16 md:py-24" style={{ background: "var(--surface, #F9FAFB)" }}>
      <div className="container">
        ...
        <div className="grid divide-x-0 divide-y md:grid-cols-3 md:divide-x md:divide-y-0"
          style={{ borderTop: "1px solid var(--hairline-mid, rgba(0,0,0,0.1))", borderBottom: "1px solid var(--hairline-mid, rgba(0,0,0,0.1))" }}
        >
          {items.slice(0, 3).map((t, i) => (
            <figure
              key={i}
              className="flex flex-col gap-6 px-0 py-10 md:px-8 md:py-8"
              style={{ borderColor: "var(--hairline-weak, rgba(0,0,0,0.08))" }}
            >
              <blockquote
